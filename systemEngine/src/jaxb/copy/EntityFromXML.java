        ...
        String propName=prdProperty.getPRDName();
            ...
            if(prdProperty.getPRDValue().isRandomInitialize()){
                ...
                switch (enumValue) {
                    case DECIMAL:
                        //propertyDefinition=new IntegerPropertyDefinition(propName, new RandomIntegerGenerator((int) (prdEnvProperty.getPRDRange().getFrom()),(int) (prdEnvProperty.getPRDRange().getTo())));
                        propertyDefinition=new IntegerPropertyDefinition(propName, ValueGeneratorFactory.createFixed(Integer.parseInt(prdProperty.getPRDValue().getInit())));
                        break;
                    case FLOAT:
                        propertyDefinition = new FloatPropertyDefinition(propName, ValueGeneratorFactory.createFixed(Float.parseFloat(prdProperty.getPRDValue().getInit())));
                        break;
                    case BOOLEAN:
                        propertyDefinition=new BooleanPropertyDefinition(propName, ValueGeneratorFactory.createFixed(Boolean.parseBoolean(prdProperty.getPRDValue().getInit())));
                        break;
                    case STRING:
                        propertyDefinition=new StringPropertyDefinition(propName, ValueGeneratorFactory.createFixed(prdProperty.getPRDValue().getInit()));
                        break;
                    default:
                        throw new IllegalStateException("Unexpected value: " + enumValue);
                }
